...
## raising of a Triops.
##
## The Headless version is experimental code
## to implement eTriops without the GUI. It is
## not considered fit for general release - 
## use at your own risk!

version = "v.1.0e - HEADLESS"
...
def resetprompt():
    prompt = input("Confirm Reset", "Are you sure you want to ERASE " + gs["name"] + " and completely start over?")
    if prompt == 'yes':
        initgame()
...
def initgame():
    ...
    global simLock
    simLock = True
    name = input("Enter Name", "What is your triops' name?")
...
def death():
    ...
    print("")
    response = input("Reset or Quit? (R/Q) ")
    if response == "R":
        initgame()
    elif response == "Q":
